...
# === DONUT CHART (Using the fixed, simple JSON structure) ===
...
# === BAR CHART (Fixed Title Placement via Layout Padding) ===
...
  "options": {
    "layout": {
      "padding": { "top": 40, "bottom": 30 } /* FIX: Increased top padding for title/labels */
    },
    "plugins": {
      "title": {
        "display": true,
...
        "anchor": "end",
        "align": "top",
        "offset": 6, /* Offset pushes numerical label above the bar */
        "clip": false,
        "padding": { "top": 6 },
        "color": "#4B286D",
...
h1 { text-align: center; color: #4B286D; font-size: 28px; margin-bottom: 5px; border-bottom: 3px solid #4B286D; padding-bottom: 10px; }
.subtitle { text-align: center; color: #777; font-size: 14px; margin-bottom: 20px; }
.summary-box { display: flex; justify-content: space-around; background-color: #f7f3fb; border-radius: 10px; padding: 15px; margin-bottom: 25px; border: 1px solid rgba(75, 40, 109, 0.2); }
.summary-item { text-align: center; padding: 0 10px; }
.summary-item span { display: block; font-size: 24px; color: #4B286D; font-weight: bold; }
.summary-item label { color: #777; font-size: 13px; }
...
th { background-color: #4B286D; color: white; padding: 12px; text-align: left; }
td { padding: 10px 12px; border-bottom: 1px solid #ddd; border-right: 1px solid #eee; color: #444444; font-size: 14px; }
...
.chart-frame { flex: 1 1 45%; background: white; border-radius: 12px; box-shadow: 0 3px 10px rgba(75, 40, 109, 0.1); padding: 15px; text-align: center; }
.footer { text-align: center; margin-top: 30px; color: #999; font-size: 13px; padding-top: 15px; border-top: 1px dashed #ddd; }
</style></head><body>
<div class='container'>
...
    <span style='color:#00A6A6;'>${success_rate}%</span><label>Success Rate</label>
  </div>
  <div class='summary-item'>
    <span style='color:#DC2626;'>${error_rate}%</span><label>Failure Rate</label>
  </div>
  <div class='summary-item'>
